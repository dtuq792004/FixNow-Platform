...
export function mapAuthError(error: AuthError | null): AuthErrorInfo {
  if (!error) {
    return { message: "An unknown error occurred", type: "unknown", canRetry: false };
  }

  const errorMessage = error.message.toLowerCase();
  const errorCode = error.status;

  // Invalid credentials — check before generic "invalid email" patterns
  if (
  ...
  // Invalid email format — exact/specific patterns only, avoids matching "invalid email or password"
  if (
