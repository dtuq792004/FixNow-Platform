...
const faqs = [
  ...
  ['Chính sách hoàn tiền của FixNow như thế nào?', 'Chúng tôi hoàn tiền 100% nếu bạn không hài lòng hoặc kỹ thuật viên không đến đúng hẹn mà không thông báo.'],
  ['Kỹ thuật viên của FixNow có được đào tạo chuyên môn không?', '100% đối tác phải trải qua xác minh lý lịch và kiểm tra tay nghề thực tế trước khi nhận việc.'],
  ...
  return (
    <GuestPageLayout>
      ...
      <section className="mx-auto grid max-w-container-max gap-xl px-lg py-xxl md:grid-cols-12">
        ...
        <aside className="space-y-lg md:col-span-4"><div className="rounded-2xl bg-primary-container p-xl text-on-primary-container shadow-lg"><h3 className="mb-md text-headline-md font-semibold">Liên hệ tổng đài</h3><p className="mb-xl opacity-90">Đội ngũ luôn sẵn sàng hỗ trợ 24/7 cho các tình huống khẩn cấp.</p><a className="flex items-center gap-md rounded-xl bg-on-primary-container p-md text-primary-container" href="tel:19001234"><span className="material-symbols-outlined text-3xl">call</span><div><small className="uppercase">Hotline khẩn cấp</small><div className="text-headline-md font-semibold">1900 1234</div></div></a></div><div className="rounded-2xl border border-outline-variant bg-white p-lg"><h4 className="mb-md text-title-lg font-semibold">Văn phòng đại diện</h4><div className="mb-md flex aspect-video items-center justify-center rounded-xl bg-primary-fixed"><span className="material-symbols-outlined text-[64px] text-primary">location_on</span></div><p className="text-on-surface-variant">Tầng 15, Tòa nhà Innovation, Công viên Phần mềm Quang Trung, Q.12, TP.HCM</p></div><div className="rounded-2xl border border-outline-variant bg-white p-lg text-center"><span className="material-symbols-outlined mb-sm text-[48px] text-primary">support_agent</span><h4 className="font-semibold">Đội ngũ hỗ trợ chuyên nghiệp</h4><p className="mt-xs text-on-surface-variant">Đã hỗ trợ hơn 50.000 khách hàng giải quyết vấn đề thành công.</p></div></aside>
      </section>
